...
There's no extra time wasted writing your features twice or more, as about 90% of react-native styles translate well into web. All it takes is to start with react-native's `View` / `Text` / `Image` primitives which will get transformed to HTML dom nodes on web.
