...
Creating a new API that checks all these boxes above can involve a lot of boilerplate.

To avoid that + have this boilerplate code generated for you, you can use the following command:
...
export const updatePostBridge = createDataBridge({
    // Assign schemas
    inputSchema: Post.partial(), // You can create, reuse or edit schemas here
    outputSchema: Post,
...
Let's use the data bridge we just created to provide bundle together the input and output types with our business logic.

You can do so by creating a new resolver file and passing the bridge as the second argument to `createResolver`, where the first argument is your resolver function that contains your business logic:
...
- 1. infer the input / arg types from the bridge's `inputSchema`
- 2. enable `parseArgs()` and `withDefaults()` helpers for validation + defaults
- 3. automatically extract the request context whether in an API route / GraphQL resolver / ...
...
### Turning resolvers into an API route handler
...
- 2. validate it
- 3. call the resolver function with the args (and e.g. token / session data)
...
### Enable GraphQL, without the hassle
...
- is auto typed with `graphql.tada` if a custom query is used (alt)
...
By default, the API generated by the `add:resolver` command is open to the public. Consider whether this is the right choice for that specific API.
