...
- [Making routes portable](#workspace-defined-routes)
- [Adding page routes](#using-the-route-generator)
- [`<Link/>`](#universal-link-component)
- [`useRouter()`](#universal-userouter-hook)
...
- ✅ **Reuse across platforms** (*auto re-exported* to **`@app/expo`** and **`@app/next`**)
...
<FileTree>
    <FileTree.Folder name="apps">
        <FileTree.Folder name="expo" defaultOpen>
            <FileTree.Folder name="app / posts/...  ← Reuses routes from /features/ in Expo" />
        </FileTree.Folder>
        <FileTree.Folder name="next" defaultOpen>
            <FileTree.Folder name="app / posts/...  ← Reuses routes from /features/ in Next" />
        </FileTree.Folder>
    </FileTree.Folder>
...
> We'll be ignoring [Data Fetching](/data-fetching) for now, but we'll come back to it in .

<div className="h-8" />

## Manually adding universal routes

> If you prefer to add routes manually, follow the steps below:
...
<FileTree>
    <FileTree.Folder name="apps / expo" defaultOpen>
        <FileTree.Folder name="app / examples / [slug] / index.tsx  ← Reuses route from /features/ in Expo" />
    </FileTree.Folder>
    <FileTree.Folder name="apps / next" defaultOpen>
        <FileTree.Folder name="app / examples / [slug] / page.tsx  ← Reuses route from /features/ in Next" />
    </FileTree.Folder>
</FileTree>
...
You can navigate using the `useRouter` hook or `Link` component from `@green-stack/navigation`. They provide familiar APIs to how navigation works in both the Next.js and `expo-router` app routers.

While they use each platform's underlying navigation system, they are not limited to just `expo-router` or `next.js`
