...
export async function getStaticProps({ params }) {
    ...
    try {
        if (branchName) {
            ...
            pageMarkdown = [
                pageMarkdown,
                `## Disclaimer`,
                `<Callout type="info" emoji="ℹ️">The \`with/${branchName}\` branch is a **git based plugin** to help test the free version of the 'FullProduct.dev' __Universal App Starterkit__. You can *look at the PR* [#${branchPR.number} - ${branchPRTitle}](${branchPR._links.html.href}) to **inspect the diff** and learn what code changes together to enable this feature... OR you can check it out locally to test / edit in isolation until ready to merge.</Callout>`,
            ].join('\n')
    ...
    const dynamicMDX = await buildDynamicMDX(pageMarkdown, { defaultShowCopyCode: true, components })
...
export const getStaticPaths = () => {
    const paths = pluginsConfig.branches.map(filePath => ({
