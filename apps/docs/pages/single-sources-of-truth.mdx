...
Think of all the places you might re-define data shapes.
...
> In this case where we check the resulting type of `z.input()`, the 'isAdmin' field will be marked as optional, as it's supposedly not defaulted to `false` yet. If we'd inspect `z.output()` or `z.infer()`, it would be marked as required since it's either provided or presumed defaulted.
...
? Generate an integration for this schema? # e.g. form, db-model
