...
```md copy
npm run add:resolver -- --args <workspacePath> <resolverName> <resolverDescription> <resolverType> <generatables> <inputSchemaTarget> <inputSchemaName> <outputSchemaTarget> <outputSchemaName> <apiPath> <formHookName>
```
