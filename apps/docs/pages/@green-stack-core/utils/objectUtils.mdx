...
Transforms all properties in a metadata object to string values only. Particularly useful for APIs like Stripe that only accept string values in metadata objects.
...
// Circular objects are skipped
const obj = { id: 1 }
obj.self = obj
stringifyMetadata(obj)
// => { id: '1' }
// (self property is skipped)
```
