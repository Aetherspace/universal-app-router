...
import { Pressable, View, Text, cn, getThemeColor } from '@app/ui'
...
export const useComponentDocs = (props: ComponentDocsProps, syncInitialState = false) => {
    ...
    const { showCode: _, ...params } = parseUrlParamsObject(queryObj)
...
export const ComponentDocsPreview = (props: ComponentDocsProps) => {
    ...
    const { componentName, previewProps, didMount, isReady, setDidMount, showCode, router, pathname } = docsUtils
    ...
    const previewPropsWithHandlers = useMemo(() => {
        const onChange = (value: any$Unknown) => {
            ...
            const query = new URLSearchParams(buildUrlParamsObject(newParams) as Record<string, string>).toString()
...
export const ComponentDocsPropTable = (props: ComponentDocsProps) => {
    ...
    const { previewProps, propSchema, propMeta, router, pathname, params, showCode } = docsUtils
    ...
    useEffect(() => {
        ...
        if (isReady) {
            const query = new URLSearchParams(buildUrlParamsObject({ ...formState.values, showCode }) as Record<string, string>).toString()
            router.push(`${pathname}?${query}`, { scroll: false })
        }
