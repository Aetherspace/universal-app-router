...
OR import them from your own predefined styled system in `@app/ui`:
...
> For convenience, we've already set up an `@app/ui` alias that points to this file for you.

Usage - e.g. `HomeScreen.tsx`

```typescript {5, 13, 22} /className/
import { Image, View, H1 } from '@app/ui'
...
```ts {4, 7, 11} /.min/ /.default/ /.nullish/ filename="User.schema.ts"
...
```ts {5, 8, 12} /.min/ /_default/ /.nullish/ filename="User.schema.ts"
...
export const User = schema('User', {
    ...
    // Zod can help you go even narrower than typescript
    name: z.string(), // <- e.g. Needs to be a string
    age: z.number(), // <- e.g. Age must be a number
...
```ts {5, 6, 10, 11} filename="healthCheck.resolver.ts"
import { z, schema } from '@green-stack/schemas'
...
export const HealthCheckInput = schema('HealthCheckInput', {
    echo: z.string().default('Hello World!').describe("Will be echo'd back"),
})

// Output defaults
export const HealthCheckOutput = schema('HealthCheckOutput', {
    echo: HealthCheckInput.shape.echo,
...
```ts {6, 7, 11, 12} filename="healthCheck.resolver.ts"
import * as z from '@green-stack/schemas/mini'
import { schema } from '@green-stack/schemas/mini'
...
// Input validation
export const HealthCheckInput = schema('HealthCheckInput', {
    echo: z._default(z.string(), 'Hello World!'),
})

// Output defaults
export const HealthCheckOutput = schema('HealthCheckOutput', {
    echo: HealthCheckInput.shape.echo,
    alive: z._default(z.boolean(), true),
    kicking: z._default(z.boolean(), true),
})
```
...
```ts {2, 4, 19, 29} /healthCheckQuery/ filename="features / @app-demo / resolvers / healthCheck.query.ts"
...
```ts {7, 8} /bridgedFetcher/1,3 /healthCheckBridge/2 filename="features / @app-demo / resolvers / healthCheck.query.ts"
