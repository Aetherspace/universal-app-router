...
> The add:route generator (Plop-based) will ask you some questions, like which resolver you'd like to fetch data from, and will generate the empty screens and routes folders in the workspace of your choosing with data fetching already set up:
...
export const getPostBridge = createDataBridge({
    ...
    allowedMethods: ['GET', 'GRAPHQL'], // <- Allow GET and GraphQL requests
})
```
...
```ts {15, 18} /createDataBridge/1,3 filename="getPost.bridge.ts"
import { Post } from '../schemas/Post.schema'
import { createDataBridge } from '@green-stack/schemas/createDataBridge'
