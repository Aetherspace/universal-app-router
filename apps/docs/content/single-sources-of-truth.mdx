...
We support three schema entrypoints: **v3** (default), **v4**, and **mini**. All work with our `schema()` wrapper and tooling.
...
A core feature of this universal starterkit is taking what works and making it better. This is why we invented `schema()` as a **tiny wrapper around zod's `z.object()`**. You can use it to define your datastructures *just once* for the entire monorepo, whether you use Zod v3, v4, or mini.
...
### Choosing v3, v4, or Mini

- **@zod/v3**: `@green-stack/schemas` — Zod v3 API, most widely used.
- **@zod/v4**: `@green-stack/schemas/v4` — Zod v4 API with extended methods.
- **@zod/mini**: `@green-stack/schemas/mini` — Zod v4-mini (lighter bundle), uses `z._default(schema, value)` instead of `.default()`.
- **Compat layer**: `@green-stack/schemas/compat` — Helper utils and types for all three (e.g. `getSchemaMetadata`, `applySchemaDefaults`, `SchemaInput`, `Meta$Schema`). Does not include schema creation — import `z` and `schema` from your chosen entrypoint.

All three work with `schema()`, introspection, and the rest of the toolkit. See the [Compat API reference](/@green-stack-core/schemas/compat) for shared utilities.
...
export const User = schema('User', {
    ...
    // Zod can help you go even narrower than typescript
    name: z.string().check(z.minLength(2)), // <- e.g. Needs to be a string with at least 2 letters
    age: z.number().check(z.minimum(18)), // <- e.g. Age must be a number of at least 18

    // Just like TS, it can help you indicate fields as optional
    isAdmin: z._default(z.boolean(), false), // <- Marked optional, defaults to false
    birthdate: z.nullish(z.date()), // = same as calling nullable(optional(...))
})
```
...
> Check the full [schema reference docs](/@green-stack-core/schemas) for all available ways to build and describe schemas across v3, v4, and mini.
...
You can also use `.default()` to provide a default value when the field isn't passed in. (Mini uses `z._default(schema, value)`.)
...
```ts {9} /z.optional(/2 /z.nullable(/2 /z.nullish(/2 /z._default(/2 filename="User.ts"
// Define a schema with optional and nullable fields
// Zod mini uses functional wrappers: z.optional(), z.nullable(), z.nullish() instead of .optional(), .nullable(), .nullish()
const User = schema('...', {

    name: z.optional(z.string()), // <- Allow undefined
    age: z.nullable(z.number()), // <- Allow null
    birthData: z.nullish(z.date()), // <- Allow both

    // mini uses _default(schema, value) instead of .default()
    isAdmin: z._default(z.boolean(), false), // <- false
})
```
...
```ts copy
import type { Metadata, Meta$Schema, Meta$Tuple, ... } from '@green-stack/schemas/compat'
```
