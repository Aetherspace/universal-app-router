...
import { z, schema, type SchemaInfer } from '../v3'
...
const healthCheck =createResolver(async () => ({}), { inputSchema: HealthCheckInput, outputSchema: HealthCheckOutput })

type healthCheck = typeof healthCheck
...
type ByID = SchemaInfer<typeof ByID>
...
test("createSchemaPlugin() applies the correct builder pattern for baseTypes (z3)", () => {
...
test("createSchemaPlugin() overrides baseType builders for zodType builders (z3)", () => {
    const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
        Boolean: (schemaKey, fieldMeta) => 'BaseBool',
        String: (schemaKey, fieldMeta) => 'BaseString',
        Number: (schemaKey, fieldMeta) => 'BaseNumber',
...
test("createSchemaPlugin() overrides zodType builder for schemaKey builders (z3)", () => {
    const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
        Boolean: (schemaKey, fieldMeta) => 'BaseBool',
        String: (schemaKey, fieldMeta) => 'BaseString',
        ZodNumber: (schemaKey, fieldMeta) => 'ZodNumber',
...
test("createSchemaPlugin() skips fields without a builder (z3)", () => {
    const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
        name: (schemaKey, fieldMeta) => 'KeyString',
    })
    expect(testPlugin.name).toBe('KeyString')
...
test("createSchemaPlugin() builder functions recieve schemaKey as the first arg (z3)", () => {
    const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
...
test("createSchemaPlugin() builder functions recieve fieldMeta as the second arg (z3)", () => {
    const testPlugin = createSchemaPlugin(UserSchema.introspect(), {
...
test("createGraphSchemaDefs() generates a valid GraphQL schema from a list of resolvers (z3)", () => {
    const { graphqlSchemaDefs } = createGraphSchemaDefs({ getUser, updateUser })
    expect(() => print(graphqlSchemaDefs)).not.toThrow()
})

test("createGraphSchemaDefs() correctly splits resolvers into queries & mutations (z3)", () => {
...
test("createGraphSchemaDefs() generates correct Query & Mutation definitions (z3)", () => {
    const { graphqlSchemaDefs } = createGraphSchemaDefs({ getUser, updateUser })
...
test("createGraphSchemaDefs() generates correct Input & Data definitions (z3)", () => {
    const { graphqlSchemaDefs } = createGraphSchemaDefs({ getUser, updateUser })
    const printedSchema = print(graphqlSchemaDefs)
...
test("createGraphSchemaDefs() generates correct nullable output data definitions (z3)", () => {
...
test("createGraphSchemaDefs() can handle resolvers created by createGraphResolver() (z3)", () => {
...
test("createGraphSchemaDefs() correctly differentiates between Int & Float (z3)", () => {
...
test("createGraphSchemaDefs() renders uuid string fields as GraphQL ID (z3)", () => {
...
test("createGraphSchemaDefs() omits sensitive fields from output type definitions (z3)", () => {
    const WithSensitive = schema('WithSensitive', {
        name: z.string(),
        secret: z.string().sensitive(),
    })
    type WithSensitive = SchemaInfer<typeof WithSensitive>
    const testResolver = createResolver(async () => {}, {
