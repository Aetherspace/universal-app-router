...
import { z, schema, type SchemaInput, type SchemaOutput } from '../v4'
...
test("Bridges created by createDataBridge infer the right argsName & query type (z4)", () => {
...
test("Bridges created by createDataBridge can build the graphql query from args & response schemas (z4)", () => {
    const graphqlQuery = healtCheckBridge.getGraphqlQuery()
    expect(print(graphqlQuery as ASTNode)).toBe(expectedQuery)
})

test("Bridges created by createDataBridge can use a custom graphql query (z4)", () => {
...
test("bridgedFetcher() can create a fetcher function from a DataBridge (z4)", async () => {
